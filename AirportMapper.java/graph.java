...
public class graph<NodeType,EdgeType extends Number>{
    /**
    ...
    protected class Path implements Comparable<Path> {
        ...
        public Path(Path copyPath, Edge extendBy) {
            Vertex newVertex = new Vertex(copyPath.start.data);
            this.start = copyPath.start;
            this.end = extendBy.target;
            this.dataSequence = new LinkedList<>();
            this.dataSequence.addAll(copyPath.dataSequence); //adding the datasequence ensures a deep copy
    ...
    protected Path dijkstrasShortestPath(NodeType start, NodeType end) {
        ...
        LinkedList<Vertex> visitedNodes = new LinkedList<>();
        
        if (!containsVertex(start) || !containsVertex(end)) throw new NoSuchElementException("start/end vertex not found");  
        Vertex firstVertex = vertices.get(start) ;
        Vertex lastVertex = vertices.get(end) ;
        Path firstNode = new Path(firstVertex);
        //insert starting node
        priorityQueue.add(firstNode); //starting vertex

       //should continue to loop until every possible path is reached
        while(!priorityQueue.isEmpty()){
            //path we are currently looking at 
            Path currentPath = priorityQueue.poll();
            if (!visitedNodes.contains(currentPath.end)){
                visitedNodes.add(currentPath.end); //check if this node has been seen
            }

            if(currentPath.end.equals(lastVertex)){
                return currentPath; //return the shortest path
            }
             for(Edge edge : currentPath.end.edgesLeaving){
              Path toRemove = null;
              //ensures shortest possible path
                for (Path path : priorityQueue){
                    if(path.end.equals(edge.target) && path.compareTo(new Path(currentPath,edge)) > 0 ){
                        toRemove = path;
                    }
                }
                if (toRemove != null){
                ...
                priorityQueue.add(new Path(currentPath, edge));
             }

            visitedNodes.add(currentPath.end);
        }
        // if (!visitedNodes.contains(vertices.get(end))){
